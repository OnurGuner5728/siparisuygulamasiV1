...
import { mockMarkets } from '@/app/data/mockdatas';
...
export default function MarketDetailPage() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (foundMarket) {
        ...
        if (foundMarket.categories && foundMarket.categories.length > 0) {
          setSelectedCategory(foundMarket.categories[0]);
        }
      }
      setLoading(false);
    }, 500);
  ...
  const filteredProducts = market?.products.filter(item => 
                ...
                <div className="border-t pt-4">
                  ...
                  <button 
                    className={`w-full mt-4 py-3 rounded-md text-white font-medium transition-colors ${
                      calculateTotal() >= market.minOrder 
                        ? 'bg-green-600 hover:bg-green-700' 
                        : 'bg-gray-400 cursor-not-allowed'
                    }`}
                    disabled={calculateTotal() < market.minOrder}
                  >
                    {calculateTotal() >= market.minOrder 
                      ? 'Siparişi Tamamla' 
                      : `Min. ${market.minOrder} TL sipariş vermelisiniz`}
                  </button>
                  
                  {calculateTotal() < market.minOrder && (
                    <p className="text-sm text-red-500 mt-2 text-center">
                      Minimum sipariş tutarına {(market.minOrder - calculateTotal()).toFixed(2)} TL kaldı
                    </p>
                  )}
                </div>
              </>
