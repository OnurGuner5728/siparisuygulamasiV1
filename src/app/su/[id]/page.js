  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.error('Su satıcısı verileri yüklenirken hata:', error);
      } finally {
        setLoading(false);
      }
    }
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center">
            ...
            <div className="mt-4 md:mt-0">
              <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                ...
                <p className="text-xl font-bold text-blue-700">{vendor.min_order_amount || 0} TL</p>
              </div>
            </div>
          </div>
          
          <p className="mt-4 text-gray-700 border-t pt-4">{vendor.description || 'Bu satıcı hakkında detaylı bilgi bulunmamaktadır.'}</p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 sticky top-4">
            ...
            {localCart.length > 0 ? (
              <>
                ...
                <Link 
                  href="/sepet" 
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-md font-medium text-center block"
                >
                  Sepete Git
                </Link>
              </>
