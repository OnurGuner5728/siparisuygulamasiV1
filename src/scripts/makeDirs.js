...
if (!fs.existsSync(scriptsDir)) {
  ...
  console.log('Scripts dizini oluşturuldu.');
} else {
  console.log('Scripts dizini zaten mevcut.');
}

// clean-mock-data.js dosyasını oluştur
const cleanMockDataContent = `const fs = require('fs');
const path = require('path');
...
const cleanMockDataPath = path.join(scriptsDir, 'clean-mock-data.js');
fs.writeFileSync(cleanMockDataPath, cleanMockDataContent);
console.log('clean-mock-data.js dosyası oluşturuldu.');

console.log('\nKullanım: node scripts/clean-mock-data.js');
console.log('Bu komut, projenizde mock veri kullanımlarını listeleyecektir.'); 